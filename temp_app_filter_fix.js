...
const attDept = document.getElementById('att-dept');
const attSem = document.getElementById('att-sem');
if (attDept && attSem) {
    attDept.addEventListener('change', refreshAttendanceSubjects);
    attSem.addEventListener('change', refreshAttendanceSubjects);
}

async function refreshAttendanceSubjects() {
  ...
  try {
    const r = await fetch(API_BASE + '/subjects/list', { headers: { 'Authorization': 'Bearer ' + ACCESS } });
    if (r.ok) {
      const allSubjects = await r.json();
      subjSelect.innerHTML = '<option value="">Select Subject</option>';
      
      allSubjects.forEach(s => {
        // Filter Logic
        let match = true;
        if (dept && s.department !== dept) match = false;
        if (sem && s.semester != sem) match = false;
...
async function init() {
  if (ACCESS) {
    ...
    if (isPage('attendance.html')) {
      ...
      await refreshAttendanceSubjects();
  ...
  try {
    ...
    if (r.ok) {
      ...
      if (data.active) {
        ...
        const stopBtn = document.getElementById('btn-stop-att');
