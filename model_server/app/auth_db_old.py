...
from sqlalchemy import Column, Integer, String, Boolean, DateTime, Text, select, delete, update
...
class BlacklistedTokenModel(Base):
    """Blacklisted token SQLAlchemy model."""
    __tablename__ = "blacklisted_tokens"
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    token = Column(Text, unique=True, nullable=False, index=True)
    blacklisted_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    reason = Column(String(255), default="revoked", nullable=False)
...
class AuthDatabase:
    """Authentication database manager using SQLAlchemy + asyncio."""
    
    def __init__(self, db_path: Optional[str] = None):
        """Initialize database."""
        if db_path is None:
            db_path = config.auth_db_path  # Use from config
    ...
    def _get_session(self) -> AsyncSession:
        """Get database session (DEPRECATED - use async context manager)."""
        # For backward compatibility with sync code
        raise NotImplementedError("Use async with auth_db.async_session() as session instead")
    
    def _init_db(self):
        """Initialize database tables (DEPRECATED - call init_db() instead)."""
        import asyncio
